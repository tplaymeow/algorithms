...
struct range { size_t left; size_t right; };
size_t uint_range_length(struct range range) { return range.right - range.left; }
...
int main() {
    ...
    for (size_t i = 0; i < flowers_count; i++) flowers[i] = read_uint();
    ...
    for (size_t i = 0; i < flowers_count; i++) {
        if (last_flower != flowers[i]) last_flower_repeats = 1;
        else if (last_flower_repeats == 1) last_flower_repeats = 2;
        else {
